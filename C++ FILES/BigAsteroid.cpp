...
ABigAsteroid::ABigAsteroid()
{
	...
	asteroidBoxCollider = CreateDefaultSubobject<UBoxComponent>(TEXT("AsteroidCollider"));
...
void ABigAsteroid::BeginPlay()
{
	...
	if (this->IsA(ABigAsteroid::StaticClass())) {
		FTimerHandle handle;
		float delay = .5f;
		
		GetWorldTimerManager().SetTimer(handle, this, &ABigAsteroid::Move, delay, false);
	}
	else {
...
void ABigAsteroid::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComponent, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (OtherActor->IsA(ABullet::StaticClass())) {
		...
		scoreManager->AddScore(pointsToGive);
		if (component) 
		{
